...
void mcx_run_simulation(Config *cfg,float *fluence,float *totalenergy){
  ...
  cl_int status = 0;
  cl_device_id devices[MAX_DEVICE];		  // devices: pre-allocated for multiple devices
  cl_event * waittoread;
  ...
  // start enqiang
  //-----------------------------------------------------------------------//
  cl_context *mcx_work_context;		  // create different contexts for different devices
  //cl_command_queue *mcxqueue;		  // compute command queue
  //enqiang: cl program for multiple contexts;
  //cl_program mcxprogram;                 // compute mcxprogram
  cl_program *mcxprogram;                 // compute mcxprogram
  ...
  // findout the working devices and assign ids to devices
  ...
  // start enqiang
  //-----------------------------------------------------------------------//
  mcx_work_context = (cl_context *)malloc(workdev*sizeof(cl_context));
  ...
  for(i=0;i<workdev;i++){
	char pbuf[100]={'\0'};
	OCL_ASSERT(((mcx_work_context[i]=clCreateContextFromType(cprops,CL_DEVICE_TYPE_ALL,NULL,NULL,&status),status)));
	//OCL_ASSERT(((mcxqueue[i]=clCreateCommandQueue(mcxcontext,devices[i],prop,&status),status)));
	OCL_ASSERT(((mcxqueue[i]=clCreateCommandQueue(mcx_work_context[i],devices[i],prop,&status),status)));
  ...
  for(i=0;i<workdev;i++){
	...
	// enqiang
	//------------------------------------------------------------------------//
	OCL_ASSERT(((gmedia[i]=clCreateBuffer(mcx_work_context[i],RO_MEM,	  sizeof(cl_uchar)*(dimxyz),	  media,&status),	  status)));
	OCL_ASSERT(((gproperty[i]=clCreateBuffer(mcx_work_context[i],RO_MEM,  cfg->medianum*sizeof(Medium),	  cfg->prop,&status), status)));
	OCL_ASSERT(((gparam[i]=clCreateBuffer(mcx_work_context[i],RO_MEM,	  sizeof(MCXParam),				  &param,&status),	  status)));

	//------------------------------------------------------------------------//
	// enqiang
	//------------------------------------------------------------------------//

	OCL_ASSERT(((gseed[i]=clCreateBuffer(mcx_work_context[i],RW_MEM, sizeof(cl_uint)*cfg->nthread*RAND_SEED_LEN,Pseed,&status),status)));
	OCL_ASSERT(((gfield[i]=clCreateBuffer(mcx_work_context[i],RW_MEM, sizeof(cl_float)*(dimxyz)*cfg->maxgate,field,&status),status)));
	OCL_ASSERT(((gdetphoton[i]=clCreateBuffer(mcx_work_context[i],RW_MEM, sizeof(float)*cfg->maxdetphoton*(cfg->medianum+1),Pdet,&status),status)));
	OCL_ASSERT(((genergy[i]=clCreateBuffer(mcx_work_context[i],RW_MEM, sizeof(float)*(cfg->nthread<<1),energy,&status),status)));
	OCL_ASSERT(((gstopsign[i]=clCreateBuffer(mcx_work_context[i],RW_PTR, sizeof(cl_uint),&stopsign,&status),status)));
	OCL_ASSERT(((gdetected[i]=clCreateBuffer(mcx_work_context[i],RW_MEM, sizeof(cl_uint),&detected,&status),status)));
	OCL_ASSERT(((gdetpos[i]=clCreateBuffer(mcx_work_context[i],RO_MEM, cfg->detnum*sizeof(float4),cfg->detpos,&status),status)));
  }
  ...
  */
  ...
  for(i=0;i<workdev;i++){
	...
	// enqiang 
	//-----------------------------------------------------------------------//
	OCL_ASSERT(((mcxprogram[i]=clCreateProgramWithSource(mcx_work_context[i], 1,(const char **)&(cfg->clsource), NULL, &status),status)));
...
*/
	  ...
	  tic1=GetTimeMillis();
	  toc+=tic1-tic0;
	  fprintf(cfg->flog,"tic1-tic_enqiang:  \t%d ms\n", tic1-tic_enqiang);
...
//	clReleaseCommandQueue(mcxqueue[devid]);
  ...
  for(cl_uint devid=0;devid<workdev;devid++)
  {
	...
	clReleaseContext(mcx_work_context[devid]);
  }

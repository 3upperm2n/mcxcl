...
void mcx_run_simulation(Config *cfg,float *fluence,float *totalenergy){
	...
	for(i=0;i<workdev;i++){
		...
		if(cfg->autopilot == 3){
			...
			//printf("gpu name : %s, capability %d.%d \n", gpu[i].name, gpu[i].major, gpu[i].minor);
		...
		//printf("autothread %ld autoblock %ld\n", gpu[i].autothread, gpu[i].autoblock);
